    ...
    if(direction === "left") {
      ...
      axios.get( 'https://tinder-9d380-default-rtdb.firebaseio.com/users.json' + queryParams)
        .then( res => {
          ...
          const urlParams = fetchedUsers[0].id 
          axios.patch('https://tinder-9d380-default-rtdb.firebaseio.com/users/' + urlParams + '/dislikedBy.json?auth=' + token, {[userId]: true})
            .then( response => {
              ...
              axios.get('https://tinder-9d380-default-rtdb.firebaseio.com/users.json' + queryParams1)
                ...
                .then( response => {
                  ...
                  const urlParams1 = authenticatedUser[0].id;
                  axios.patch('https://tinder-9d380-default-rtdb.firebaseio.com/users/' + urlParams1 + '/disliked.json?auth=' + token, {[swipedUserId]: true})
    ...
    //if the user swiped right
      ...
      axios.get( 'https://tinder-9d380-default-rtdb.firebaseio.com/users.json' + queryParams)
        .then( res => {
          ...
          const userId = localStorage.getItem("userId");
          const urlParams = fetchedUsers[0].id 
          axios.patch('https://tinder-9d380-default-rtdb.firebaseio.com/users/' + urlParams + '/likedBy.json?auth=' + token, {[userId]: true})
            .then( response => {
              //now we need to add to the logged in user saying that we swiped left on the shown user, for that we need the key to the object that represents the logged in user,
              //for that we need to grab the logged in user
              const queryParams1 = '?auth=' + token + '&?orderBy="userId"&equalTo="' + userId + '"';
              axios.get('https://tinder-9d380-default-rtdb.firebaseio.com/users.json' + queryParams1)
                ...
                .then( response => {
                  ...
                  const urlParams1 = authenticatedUser[0].id;
                  axios.patch('https://tinder-9d380-default-rtdb.firebaseio.com/users/' + urlParams1 + '/liked.json?auth=' + token, {[swipedUserId]: true})
